...
const authLimiter = rateLimit({
  windowMs: 10 * 60 * 1000,
  max: 10,
  message: { message: '요청이 너무 많습니다. 10분 후 다시 시도해주세요.' },
...
router.post('/forgot-password', authLimiter, async (req, res) => {
  try {
    ...
    const user = await prisma.user.findUnique({ where: { email } });
    if (!user) return res.status(404).json({ message: '해당 이메일은 등록되어있지 않습니다.' });
...
router.post('/verify-reset-code', async (req, res) => {
  try {
    ...
    if (!record) return res.status(400).json({ message: '인증번호가 존재하지 않습니다.' });
...
router.post('/reset-password', async (req, res) => {
  try {
    ...
    if (new Date() > record.expiresAt) {
      await prisma.passwordResetCode.deleteMany({ where: { email } });
      return res.status(400).json({ message: '인증이 만료되었습니다. 처음부터 다시 진행해주세요.' });
    }
